...
from enigma import RT_HALIGN_LEFT, RT_HALIGN_CENTER, eListboxPythonMultiContent, eServiceReference, eServiceCenter, gFont, iServiceInformation
...
def FileEntryComponent(name, absolute = None, isDir = False):
	res = [ (absolute, isDir) ]
	res.append((eListboxPythonMultiContent.TYPE_TEXT, 70, 2, 1000, 35, 0, RT_HALIGN_LEFT, name))
	if isDir:
		...
		if EXTENSIONS.has_key(extension):
			...
			png = None
	if png is not None:
		res.append((eListboxPythonMultiContent.TYPE_PIXMAP_ALPHABLEND, 12, 3, 35, 35, png))
	return res
			...
			for x in files:
				...
				if (self.matchingPattern is None) or (EXext in self.matchingPattern):
					if nx is None:
						...
						res = [ (x, False) ]
						res.append((eListboxPythonMultiContent.TYPE_TEXT, 40, 2, 1000, 35, 0, RT_HALIGN_LEFT, name + " [" + self.getTSLength(path) + "]"))
